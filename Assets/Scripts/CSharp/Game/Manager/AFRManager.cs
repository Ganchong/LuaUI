...
public class AFRManager : Singleton<AFRManager>
{
	public void RegisterFunc ()
	{
		...
		UIRawImage.LoadTextureFunc = (path, rawImage, call) => {
			...
			TexturePool.Instance.Load (ResourceHelper.TEXTUREPATH + path, (mainTex, alphaTex) => {
				if (rawImage == null || rawImage.IsDestroyed ())
					return;
				if (mainTex == null) {
		...
		UIImage.LoadSprite = (atlasName, name, uiImage, call) => {
			ResourceHelper.Instance.LoadResDataAsync (ResourceHelper.ATLASPATH + atlasName, (res) => {
