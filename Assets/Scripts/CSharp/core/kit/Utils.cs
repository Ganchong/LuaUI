...
{	
	...
	public static bool saveDataFileList(string path, Dictionary<string, UpdateHelper.FileInfo> dicFileInfos)
	{
		try
		{
			CheckTargetPath(path);
			Log.debug("copy dataFile file");
			FileStream stream = new FileStream(path, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
			StreamWriter writer = new StreamWriter(stream);
			foreach(var pair in dicFileInfos)
			{
